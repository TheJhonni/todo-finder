...
export default function RepliesComments({ replies, commentAuthor }) {
  ...
  const fetchReplies = (id) => {
    ...
    setTimeout(() => {
      setMount(true);
      fetch(`http://localhost:5000/myPosts/${id}/comments?`)
        .then((res) => {
          return res.json();
        })
        .then(({ replies }) => {
  ...
  const deleteReply = (id) => {
    if (window.confirm("Are you sure to delete this reply?")) {
      setMount(true);
      fetch(`http://localhost:5000/myPosts/comments/replies/${id}`, {
        method: "DELETE",
            ...
            >
              ...
              <div className="flex flex-col flex-grow ml-4">
                <div className="flex items-center">
                  ...
                  <span className="ml-1">@ random author</span>

                  <span className="text-sm ml-10">at: {reply.replyDate}</span>
                </div>
